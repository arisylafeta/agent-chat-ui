...
export function ArtifactSidebar({
  ...
  const isLargeScreen = useMediaQuery("(min-width: 55rem)");
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          animate={{ opacity: 1 }}
          className={cn(
            "fixed top-0 left-0 z-50 grid h-dvh w-dvw grid-cols-1 min-[55rem]:grid-cols-[400px_1fr] bg-transparent pointer-events-none overflow-x-hidden",
            className,
          )}
        ...
        >
          ...
          <motion.div
            ...
            className="col-start-1 min-[55rem]:col-start-2 z-20 flex h-dvh min-w-0 max-w-full flex-col overflow-x-hidden overflow-y-auto border-zinc-200 bg-gray-soft min-[55rem]:border-l dark:border-zinc-700 dark:bg-muted pointer-events-auto relative"
            data-artifact-panel
            exit={{
              opacity: 0,
              scale: 0.5,
