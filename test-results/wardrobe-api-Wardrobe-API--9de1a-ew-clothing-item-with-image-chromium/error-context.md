...
- Name: Wardrobe API >> POST /api/wardrobe >> should create a new clothing item with image
- Location: /Users/admin/Desktop/AI/Reoutfit/app-reoutfit/tests/e2e/wardrobe-api.spec.ts:51:5
...
Expected: 201
Received: 404
    at /Users/admin/Desktop/AI/Reoutfit/app-reoutfit/tests/e2e/wardrobe-api.spec.ts:75:33
```
...
```ts
   ...
   5 | const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000';
   6 | const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || '';
   7 | const SUPABASE_ANON_KEY = process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || '';
   8 |
   9 | // Helper to create a test user and get auth token
   10 | async function getAuthToken() {
   11 |   if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
   12 |     throw new Error(
   13 |       'Missing Supabase credentials. Please set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY in .env.local'
   14 |     );
   15 |   }
   16 |
   17 |   const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
   18 |   
   19 |   // Sign in with test user (you'll need to create this user in your Supabase project)
   20 |   const { data, error } = await supabase.auth.signInWithPassword({
